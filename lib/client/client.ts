...
export async function postSqlRequest(
  sql: string,
  cfg: ClientConfig
): Promise<SqlRequestResponse> {
  ...
  const { commit, table } = await commitToQuery(
    sql,
    cfg.knownTables,
    cfg.circuitParams
  );
...
export async function getSqlRequest(
  ...
  console.log("new res client: ", res);

  clearTimeout(id);
  console.log("ok res2: ", res.ok);
  if (!res.ok) {
    return {
      ready: true,
      error: res.statusText,
  ...
  const { commit, table, type } = await commitToQuery(
    sql,
    cfg.knownTables,
    cfg.circuitParams
  );
  ...
  const tableHeader = new Map<string, bigint>(

...
export async function listenToChain(address: string) {
  ...
  zkSqlContract.on(
    "RequestPosted",
    (address: any, argsCommitment: BigNumber) => {
  ...
  zkSqlContract.on(
    "TableUpdated",
    (table: string, newCommitment: BigNumber) => {
...
export async function requestAllTables(
  provider: Provider
  ...
  let eventIFace = new utils.Interface([
    "event TableCreated(string table, uint256 commitment)",
  ]);
  let tables = await provider
    .getLogs({

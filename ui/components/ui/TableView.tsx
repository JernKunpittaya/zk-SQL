...
export const TableView: FC<TableViewProps> = ({ tableName, columnNames }) => {
    ...
    fetch('/api/table/prepare', {
      ...
      .then(({ error, commitExpected, table, commit }) => {
        ...
        if (commitExpected) {
          ...
          const contractOwner = contract.connect(
            provider.getSigner()
          ) as IZkSQL;
          return contractOwner
            .request(table, BigInt(commit))
            .then(() => values.sql);
        }
        return values.sql;
      })
      .then((sql) => {
        ...
        showFeedback({
          title: 'Please wait',
          status: 'info',
          description: 'Connecting to the node...',
          duration: null,
        });
        ...
        fetch('/api/table/request', {
          ...
          .then(({ token, tableCommit, error }) => {
            console.log('token from postsql: ', token);
            if (error != null) {
              showFeedback({
                title: 'Error occurred',
                status: 'error',
  ...
  async function poll(sql: string, tableCommit: string, token: string) {
    let { ready, error, selected, type, proof, publicSignals } = await fetch(
      '/api/table/query',
      {
        method: 'POST',
        body: JSON.stringify({
    ...
    if (!ready) {
      ...
      reqMade++;
      showFeedback({
        description: `Waiting for data${[...Array((reqMade % 3) + 1)]
          .map((_) => '.')
          .join('')}`,
      });

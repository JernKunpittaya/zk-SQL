...
export class RequestController {
  ...
  async updatePayload(
    ...
    try {
      ...
      (async () => {
        try {
          console.log("request param db: ", db);
          console.log("request param payload: ", payload.sql);
          console.log("request param argscomm: ", argsCommit);
          console.log("request param circuit: ", circuitParams);
          return {
            res: await execQuery(
              db,
              payload.sql,
        ...
        if (type != "select") {
          ...
          await zkSqlContract.execRequest(
            getSqlOpcode(type),
            argsCommit,
            newCommitment,
            res.solidityProof!
        ...
        requests.set(token, {
          ...
          changeCommit:
            type != "select" ? res.publicInputs![0].toString() : undefined,
          proof: res.proof!,
        });
